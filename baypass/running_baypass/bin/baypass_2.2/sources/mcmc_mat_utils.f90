...
!REAL (kind=8), SAVE  :: c, d !!!!!=> not thread proof
REAL (kind=8)  :: c, d
...
type(mt_state), intent(inout) :: this
REAL (kind=8), INTENT(IN)    :: s
!REAL (kind=8), SAVE          :: a, p, c, uf, vr, d !!!!!=> not thread proof
REAL (kind=8)          :: a, p, c, uf, vr, d 
...
!REAL (kind=8), SAVE       :: d, f, h, t, c !!!!!=> not thread proof
!LOGICAL, SAVE    :: swap !!!!!=> not thread proof
REAL (kind=8)       :: d, f, h, t, c 
LOGICAL    :: swap 
...
FUNCTION random_binomial2(n, pp,this) RESULT(ival)
implicit none
type(mt_state), intent(inout) :: this
REAL (kind=8), INTENT(IN)    :: pp
INTEGER , INTENT(IN) :: n
REAL (kind=8)    :: alv, amaxp, f, f1, f2, u, v, w, w2, x, x1, x2, ynorm, z, z2
INTEGER  :: i, ix, ix1, k, mp,ival
!INTEGER , SAVE   :: m                           !!!!!=> not thread proof
!REAL (kind=8), SAVE  :: p, q, xnp, ffm, fm, xnpq, p1, xm, xl, xr, c, al, xll,  &
!                   xlr, p2, p3, p4, qn, r, g    !!!!!=> not thread proof
INTEGER    :: m
REAL (kind=8)  :: p, q, xnp, ffm, fm, xnpq, p1, xm, xl, xr, c, al, xll,  &
                   xlr, p2, p3, p4, qn, r, g
!     .. Executable Statements ..
!*****SETUP, PERFORM ONLY WHEN PARAMETERS CHANGE

  p = MIN(pp, 1.-pp) ; q = 1. - p ; xnp = n * p

IF (xnp > 30.) THEN
    ffm = xnp + p ; m = int(ffm) ; fm = m ; xnpq = xnp * q !modif: m=ffm <-m=int(ffm)
    p1 = INT(2.195*SQRT(xnpq) - 4.6*q) + 0.5
    xm = fm + 0.5 ; xl = xm - p1 ; xr = xm + p1
...
!     PARALLELOGRAM REGION
  IF (u <= p2) THEN
    ...
    ix = nint(x) !modif: x <-int(x)
  ELSE
!     LEFT TAIL
    IF (u <= p3) THEN
      ix = nint(xl + LOG(v) / xll)
      IF (ix < 0) GO TO 20
      v = v * (u-p2) * xll
    ELSE
!     RIGHT TAIL
      ix = nint(xr - LOG(v) / xlr )
      IF (ix > n) GO TO 20
      v = v * (u-p3) * xlr
    END IF
  END IF
...
!     SQUEEZING USING UPPER AND LOWER BOUNDS ON LOG(F(X))
  amaxp = (k/xnpq) * ((k*(k/3. + .625) + .1666666666666)/xnpq + 0.5)
  ynorm = -k * k / (2.*xnpq)
  alv = LOG(v)
...
!     THE FINAL ACCEPTANCE/REJECTION TEST
  x1 = ix + 1 ; f1 = fm + 1. ; z = n + 1 - fm
  w = n - ix + 1. ; z2 = z * z ; x2 = x1 * x1
  f2 = f1 * f1 ; w2 = w * w
  IF (alv - (xm*LOG(f1/x1) + (n-m+0.5)*LOG(z/w) + (ix-m)*LOG(w*p/(x1*q)) +    &
      (13860.-(462.-(132.-(99.-140./f2)/f2)/f2)/f2)/f1/166320. +               &
      (13860.-(462.-(132.-(99.-140./z2)/z2)/z2)/z2)/z/166320. +                &
      (13860.-(462.-(132.-(99.-140./x2)/x2)/x2)/x2)/x1/166320. +               &
      (13860.-(462.-(132.-(99.-140./w2)/w2)/w2)/w2)/w/166320.) > 0.) THEN
    GO TO 20
  ELSE
    GO TO 110
  END IF
...
!     INVERSE CDF LOGIC FOR MEAN LESS THAN 30
    qn = q ** n ; r = p / q ; g = r * (n+1)
  90 ix = 0
  f = qn ; u=grnd(this) 
  100 IF (u >= f) THEN
    IF (ix > 110) GO TO 90
    u = u - f ; ix = ix + 1 ; f = f * (g/ix - r)
    GO TO 100
  END IF
END IF
110 IF (pp > 0.5) ix = n - ix
ival = ix
RETURN
...
subroutine elapse_time(t1,niter_done,niter_tot)
implicit none
real , intent(in) :: niter_done,niter_tot
integer (kind=8), intent(in) :: t1(3)
integer (kind=8) :: t2(3)
integer :: tmp_jhms(4)
real (kind=8) :: tmp,tmp_nsec

call system_clock(t2(1),t2(2),t2(3))
tmp_nsec=real(t2(1)-t1(1))/real(t2(2))
if(niter_done<0) then
 tmp=tmp_nsec
 tmp_jhms(1)=max(0,floor(tmp/86400.)) ; tmp = tmp - real(tmp_jhms(1))*86400.
 tmp_jhms(2)=max(0,floor(tmp/3600.)) ; tmp = tmp - real(tmp_jhms(2))*3600.
 tmp_jhms(3)=max(0,floor(tmp/60.))
 tmp_jhms(4) = max(0,floor(tmp - real(tmp_jhms(3))*60.))
 write(*,'(A,4(i2,A))')   '    Elapsed Time: ',tmp_jhms(1),' d ',tmp_jhms(2), ' h ',tmp_jhms(3),' min ',tmp_jhms(4),' sec'
else
 tmp=tmp_nsec/niter_done
 tmp=tmp*(niter_tot-niter_done) + 10. !on rajoute 10 secondes pour tenir compte des calcul finaux (et faciliter arrondis
 tmp_jhms(1)=max(0,floor(tmp/86400.)) ; tmp = tmp - real(tmp_jhms(1))*86400.
 tmp_jhms(2)=max(0,floor(tmp/3600.)) ; tmp = tmp - real(tmp_jhms(2))*3600.
 tmp_jhms(3)=max(0,floor(tmp/60.))
 tmp_jhms(4) = max(0,floor(tmp - real(tmp_jhms(3))*60.))
write(*,'(A,4(i2,A))')     '   (Estimated) Remaining Time: ',tmp_jhms(1),' d ',tmp_jhms(2), ' h ',tmp_jhms(3),' min ',tmp_jhms(4),' sec' 
end if
...
!  integer, save :: indx(8000) !!!!!=> not thread proof
  integer :: indx(8000) !!!!ATTENTION DIMENSION FIXEE A 8000 mais visiblement ne doit pas etre inferieur à la taille de a (i.e., n)
  real (kind=8) :: d
  integer :: n,m,i
