...
function logl_ij_beta(Y_obs,N_obs,A_ij,LOG_CN)
 ...
 if(A_ij> eps .and. (1.-A_ij)>eps .and. N_obs>0) then
...
if(y_cur/=0 .and. y_cur/=popsize)  tmp_logcur=log_binomial_coef(popsize,y_cur) + &
                                              real(y_lect)*log(dum_real) + real(n_lect-y_lect)*log(1.-dum_real)
...
if(y_out/=y_cur) then
!vrais_prop
 dum_real=real(y_out)/real(popsize)
 if(y_out/=0 .and. y_out/=popsize)  tmp_lognew=log_binomial_coef(popsize,y_out) + &
                                               real(y_lect)*log(dum_real) + real(n_lect-y_lect)*log(1.-dum_real)
...
!!!!
subroutine update_alpha_indseq (this,ind_pl_val,gind_like_cur,a_cur,a_std_cur,pi_val,cov_val,mu_up,var_up,delta_a,accept,a_out,a_std_out,gind_like_out) !delta_a=taille de l'intervalle autour de la proposal de a
implicit none

!on fait une marche aleatoire pure
type(mt_state), intent(inout) :: this
real (kind=8), intent(in),dimension(:,:) :: ind_pl_val !dimensions= nind de la pop, 3 (i.e., pl homo ref, hetero, homo alt) + 1 (current genotype likilihood)
real (kind=8), intent(in) :: a_cur,gind_like_cur,a_std_cur,pi_val,cov_val,mu_up,var_up, delta_a

integer, intent(out) :: accept
real (kind=8), intent(out) :: a_out,a_std_out,gind_like_out

real (kind=8) :: b_inf, b_sup,diff_log,hwe_prob(3),dum_freq,eps=1e-8 !Attention: eps doit matcher avec celui utilise pour initialisation
integer :: tmp_nind,ii 

accept=1 ;gind_like_out=0.
!on echantillonne la proposal
b_inf=a_std_cur - delta_a/2 ; b_sup=a_std_cur + delta_a/2
a_std_out=b_inf+grnd(this)*(b_sup - b_inf)
a_out=pi_val + cov_val + a_std_out*sqrt(pi_val*(1.0-pi_val)) 
...
!!!!!
!HWE probs
dum_freq=max(eps,min(1.-eps,a_out))
hwe_prob(1)=dum_freq*dum_freq ; hwe_prob(2)=2.*(1.-dum_freq)*dum_freq ; hwe_prob(3)=(1.-dum_freq)*(1.-dum_freq)

tmp_nind=size(ind_pl_val,1)
do ii=1,tmp_nind
 gind_like_out=gind_like_out+log(dot_product(ind_pl_val(ii,:),hwe_prob))
end do
diff_log=diff_log + gind_like_out - gind_like_cur

if(log(grnd(this))>diff_log) then
 accept=0 ; a_out=a_cur ; a_std_out=a_std_cur ; gind_like_out=gind_like_cur
end if

end subroutine update_alpha_indseq
...
subroutine update_pi (this,p_cur,vect_a_cur,cov_vals,lambda_mat,root_cnt,beta_params,delta_p,accept,p_out,dum_real_pop) 
implicit none

integer, intent(in) :: root_cnt(2)
type(mt_state), intent(inout) :: this
...
diff_log=(beta_params(1) - coef + root_cnt(1))*(log(p_out) - log(p_cur)) + &
         (beta_params(2) - coef + root_cnt(2))*(log(1.0-p_out) - log(1.0-p_cur)) + &
...
subroutine update_nichc (this,nmrk,c_cur,sumsqAtld,delta_c,accept,c_out)
implicit none
type(mt_state), intent(inout) :: this
integer, intent(in) :: nmrk
real (kind=8), intent(in) :: delta_c , c_cur,sumsqAtld ! somme des aij_tilde^2 : aij_tilde=(aij-pi)/sqrt(pi(1-pi))

real (kind=8), intent(out) :: c_out
integer, intent(out) :: accept !0 si accepte, 1 sinon

integer :: tmp_i
real (kind=8) :: b_inf, b_sup, inv_fwd, inv_bwd, diff_log

 accept=1
 b_inf=max(1e-5,c_cur-delta_c/2) ; b_sup=min(1.,c_cur+delta_c/2)
 c_out=b_inf+grnd(this)*(b_sup - b_inf)
 inv_fwd=b_sup - b_inf
 inv_bwd=min(1.,c_out+delta_c/2) - max(1e-5,c_out-delta_c/2)
 diff_log= log(inv_fwd) - log(inv_bwd) + 0.5*nmrk*(log(c_cur) - log(c_out)) + 0.5*sumsqAtld*(1./c_cur - 1./c_out)
 if(log(grnd(this))>diff_log) then
   accept=0 ; c_out=c_cur
 end if

 end subroutine update_nichc

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!UPDATE DES PHENO_Z!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

subroutine up_phenoz(this,phenoobs,zprior,tmp_nmrk,tmp_npop,phi_j,mat_alpha,vect_deltabeta,vect_pi,chol_lambda_in,cov_val_in,phi_out)
!on ne parallelise la partie echantillonnage car pas sur qu'on gagne du temps + gestion dimension du vecteur mts en entree/sortie (i.e., 0:threds-1 VS 1:threads
implicit none

type(mt_state), intent(inout) :: this
real (kind=8), intent(inout), dimension(:,:) :: cov_val_in
real (kind=8), intent(in), dimension(:)   :: phi_j,vect_pi,vect_deltabeta,zprior !zprior=c(mu,tau,sd)
real (kind=8), intent(in), dimension(:,:) :: phenoobs,chol_lambda_in,mat_alpha
integer , intent(in) :: tmp_nmrk,tmp_npop

real (kind=8),intent(out), dimension(:) :: phi_out

real (kind=8) :: mat_ddot_alpha(tmp_nmrk,tmp_npop),sum_betai2,fullcdttau,fullcdtmean
integer :: tmp_i

 mat_ddot_alpha=mat_alpha
 !$OMP PARALLEL
 !$OMP do schedule(guided) 
 do tmp_i=1,tmp_nmrk
  mat_ddot_alpha(tmp_i,:)=(mat_ddot_alpha(tmp_i,:) - vect_pi(tmp_i) - cov_val_in(tmp_i,:) + vect_deltabeta(tmp_i)*phi_j)/sqrt(vect_pi(tmp_i)*(1.-vect_pi(tmp_i)))
  mat_ddot_alpha(tmp_i,:)=matmul(chol_lambda_in,mat_ddot_alpha(tmp_i,:))
 end do
 !$OMP END do   
 !$OMP END PARALLEL  

 sum_betai2=sum(vect_deltabeta*vect_deltabeta) 
 do tmp_i=1,tmp_npop
  fullcdttau=zprior(2) + phenoobs(tmp_i,2) + sum_betai2
  fullcdtmean=(zprior(1)*zprior(2) + phenoobs(tmp_i,1)*phenoobs(tmp_i,2) + sum(mat_ddot_alpha(:,tmp_i)*vect_deltabeta))/fullcdttau
  phi_out(tmp_i)=fullcdtmean + random_normal(this)/sqrt(fullcdttau)
 end do

 !$OMP PARALLEL
 !$OMP do schedule(guided) 
 do tmp_i=1,tmp_nmrk
  cov_val_in(tmp_i,:)=cov_val_in(tmp_i,:) + vect_deltabeta(tmp_i)*(phi_out - phi_j)
 end do
 !$OMP END do   
 !$OMP END PARALLEL  

end subroutine up_phenoz
