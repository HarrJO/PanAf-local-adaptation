...
!          IPvalue=255    ! ilength of verb value
      ...
      integer, parameter,public :: IPvalue=1000                        ! ilength of verb value //modifié: init=255
      integer, parameter,public :: ic=100                               ! number of entries in language dictionary //modifié init=30
...
!~          call send_message("E-R-R-O-R: UNKNOWN OPTION "//name)
         ii=index(name,"_")
         call send_message("ERROR: Option -"//name(ii+1:len_trim(name))//" is unknwon (see -help for a list of allowed options)")         
!~          if(ii > 0)then
        !    call send_message(name(:ii-1)//" parameters are")
!~             call send_message(" Options are:")
!~             do i10=1,ic
...
!~             enddo
!~          endif
         stop !rajout WHAM pour arreter si pas option: lignees commentees si dessus pour pas rendre visible la liste des options
         return
      endif
...
!~       print "("" #kracken>:"",a)", msg(:len_trim(msg)) ! echo mode
      print "(a)", msg(:len_trim(msg)) ! echo mode
